//! Módula para conectar com a conta Xbox Live.
//!
...
use crate::constants::{
    XBOX_MSA_AUTHORIZE_ENDPOINT, XBOX_MSA_REDIRECT_URI, XBOX_MSA_SCOPES, XBOX_MSA_TOKEN_ENDPOINT,
    XBOX_USER_AUTH_ENDPOINT, XBOX_XSTS_AUTH_ENDPOINT,
};
    ...
    pub(crate) async fn ensure_valid_xsts(&self) -> Result<(String, String, String), AppError> {
        ...
        let refresh_token = stored.refresh_token.clone().ok_or_else(|| {
            AppError::OAuthTokenNotFound(
                "xbox_live: token expirado e sem refresh_token, é necessário novo login"
                    .to_string(),
            )
        })?;
