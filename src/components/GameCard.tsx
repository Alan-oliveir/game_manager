...
import {Clock, Edit, Heart, ImageOff, MoreVertical, Star, Trash2} from "lucide-react";
    ...
    return (
        ...
        >
            ...
            <div className="relative aspect-[2/3] overflow-hidden bg-muted">
                {/* Se tem URL E não deu erro, mostra img. Senão, mostra fallback */}
                {game.cover_url && !imageError ? (
                    <img
                        ...
                        loading="lazy"
                        onError={() => setImageError(true)} // <--- O segredo está aqui
                    />
                ) : (
                    /* Fallback Visual */
                    <div
                        className="w-full h-full bg-gradient-to-br from-sidebar-accent via-muted to-background flex flex-col items-center justify-center p-4 text-center">
            ...
            <div className="p-4 flex flex-col gap-2 flex-1">
                ...
                <div className="flex gap-2 flex-wrap mt-auto pt-1">
                    {game.genre && (
                        <span
                            className="px-2 py-1 bg-primary/10 text-primary text-xs rounded-md font-medium whitespace-nowrap">
              {game.genre}
            </span>
                    )}
                    {game.platform && (
                        <span
                            className="px-2 py-1 bg-secondary text-secondary-foreground text-xs rounded-md font-medium whitespace-nowrap">
              {game.platform}
            </span>
                    )}
                </div>
