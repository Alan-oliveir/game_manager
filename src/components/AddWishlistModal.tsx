...
import { wishlistService, SteamSearchResult } from "../services/wishlistService";
...
export default function AddWishlistModal({ isOpen, onClose, onSuccess }: AddWishlistModalProps) {
    ...
    <div className="max-h-75 overflow-y-auto custom-scrollbar space-y-2">
        {results.map(game => (
                <div key={game.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 border border-transparent hover:border-border transition-colors">
            <div className="w-12 h-12 shrink-0 bg-background rounded overflow-hidden">
                {game.tiny_image ? (
                        <img src={game.tiny_image} alt="" className="w-full h-full object-cover" />
) : (
        <div className="w-full h-full flex items-center justify-center"><ImageOff size={16}/></div>
)}
    </div>
...
>
    {addingId === game.id ? <Loader2 className="animate-spin h-4 w-4"/> : <Plus size={18} />}
    </Button>
    </div>
    ))}
    {results.length === 0 && !loading && query && (
        <p className="text-center text-sm text-muted-foreground py-4">Nenhum jogo encontrado.</p>
    )}
    </div>
    </DialogContent>
    </Dialog>
