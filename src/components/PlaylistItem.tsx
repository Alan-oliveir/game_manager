        ...
        >
            ...
            <div className="relative h-16 w-12 shrink-0 rounded overflow-hidden bg-muted cursor-pointer group/img">
                {game.cover_url ? (
                    ...
                    <div className="h-full w-full flex items-center justify-center text-[8px]">Sem Capa</div>
                )}
                {/* Overlay Play Mini */}
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover/img:opacity-100 flex items-center justify-center transition-opacity" onClick={(e) => {e.stopPropagation(); onPlay();}}>
            ...
            <div className="flex-1 min-w-0 cursor-pointer">
                <h4 className="font-semibold truncate group-hover:text-primary transition-colors">{game.name}</h4>
                <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
                    <span className="bg-secondary px-1.5 py-0.5 rounded text-secondary-foreground">{game.genre || "Geral"}</span>
                    <span>•</span>
                    <span>{game.playtime > 0 ? `${Math.floor(game.playtime / 60)}h jogadas` : "Nunca jogado"}</span>
                </div>
            </div>
