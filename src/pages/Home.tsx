import {Clock, Gamepad2, Heart, Library, Sparkles, TrendingUp} from "lucide-react";
...
export default function Home({games}: HomeProps) {
    ...
    const suggestion = lowPlaytimeGames.length > 0
    ...
    return (
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="mb-8">
                ...
                <p className="text-muted-foreground">Bem-vindo de volta! Veja um resumo da sua biblioteca.</p>
            </div>
            ...
            {suggestion && (
                <div
                    className="bg-gradient-to-r from-purple-500/10 via-pink-500/10 to-blue-500/10 border border-purple-500/20 rounded-xl p-6 mb-8">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex-1">
                            <h3 className="text-lg font-medium mb-1">Que tal jogar {suggestion.name}?</h3>
                            <p className="text-sm text-muted-foreground">
                                {suggestion.playtime ? `Você já jogou ${suggestion.playtime}h` : "Você ainda não experimentou este jogo"} -
                                hora de dar uma chance!
                            </p>
                        </div>
                        <button
                            className="px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:opacity-90 transition-opacity whitespace-nowrap">
                            Jogar Agora
                        </button>
                    </div>
                </div>
                                ...
                                <div className="aspect-[3/4] bg-muted rounded-lg mb-2 overflow-hidden relative">
                                    {game.cover_url ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center">
                                            <Library size={32} className="text-muted-foreground opacity-50"/>
                                        </div>
                                    )}
                                    <div
